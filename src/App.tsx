...
export default function App() {
  ...
  return (
    <div className='container grid place-content-center min-h-screen text-center mx-auto'>
      <h1 className='text-3xl font-bold'>Who Knows U</h1>
      <h2 className='mt-5'>
      ...
      <div className='flex mb-1 mt-10 items-center gap-1 px-10 border rounded-full'>
        <aside className='relative inline-block' onClick={() => setOpen(!open)}>
          ...
          <section
            className={`${
              open ? 'block' : 'hidden'
            } absolute w-96 px-4 py-3 z-10 border h-60 overflow-y-auto bg-white`}
          >
        ...
        <input
          className='flex-1 p-3 border border-transparent max-w-full  focus:outline-none '
          type='tel'
          pattern='[0-9]*'
      ...
      {validatePhone(phone.number) && (
        <button className='max-w-max mx-auto flex gap-2 items-center mt-5 px-10 py-2 bg-green-500 text-white rounded-full'>
          <svg
